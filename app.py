...
async def handle_query():
    ...
    asyncClient = AsyncOpenAI(base_url="xx",
                          api_key="Not needed by Compass API, only for passing the validation of the SDK",
                          default_headers={"api-key": "**************************"})

    service_id="chat-gpt"
    kernel.add_service(OpenAIChatCompletion(service_id=service_id,ai_model_id="gpt-4",async_client=asyncClient))
